...
###############################################################################
#                        Blind (text-only) evaluation                         #
###############################################################################

def blind_eval(prompt: str, model: str, client: OpenAI):
    msgs = [
...
def vqa_parsing_with_scot(prompt: str, img: Image.Image, *, novel_view_img: Image.Image, sys_prompt: str, model: str, client: OpenAI):
    base64_img = encode_pil_image(img)
    base64_novel_view_img = encode_pil_image(novel_view_img)
    msgs = [
        {"role": "system", "content": sys_prompt},
        {
            "role": "user",
            "content": [
                {"type": "text", "text": prompt},
                {"type": "image_url", "image_url": {"url": f"data:image/jpeg;base64,{base64_img}", "detail": "high"}},
                {"type": "image_url", "image_url": {"url": f"data:image/jpeg;base64,{base64_novel_view_img}", "detail": "high"}},
            ],
        },
    ]
    return _chat_with_retry(msgs, model, client)
